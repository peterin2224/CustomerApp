...
paymentsGatewayList(List<Widget> list, double windowWidth, int _currVal, Function(int) setCurVal,
  ...
  var _total = '${basket.getTotal(true).toStringAsFixed(appSettings.symbolDigits)}';
  if (applePayEnable && Platform.isIOS) {
    list.add(Container(
        ...
        child: ApplePayButton(
          ...
          onPaymentResult: (paymentResult) {
            debugPrint(paymentResult.toString());
            debugPrint(paymentResult["paymentMethod"]["displayName"]);
            onSuccess(
                "ApplePay ${paymentResult["paymentMethod"]["displayName"]}");
          },
  ...
  if (googlePayEnable && Platform.isAndroid) {
    list.add(Container(
        margin: EdgeInsets.only(left: 10, right: 10, bottom: 10),
        child: GooglePayButton(
          ...
          style: theme.darkMode ? GooglePayButtonStyle.white : GooglePayButtonStyle.black,
          type: GooglePayButtonType.pay,
          //margin: const EdgeInsets.only(top: 15.0),
          onPaymentResult: (paymentResult) {
            debugPrint(paymentResult.toString());
            debugPrint(paymentResult["paymentMethodData"]["description"]);
            onSuccess("GooglePay ${paymentResult["paymentMethodData"]["description"]}");
          },
          loadingIndicator: const Center(
            child: CircularProgressIndicator(),
          ),
