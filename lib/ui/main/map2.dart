...
class MapInfoScreen extends StatefulWidget {
  ...
  MapInfoScreen({required this.callback});
  @override
...
class _MapInfoScreenState extends State<MapInfoScreen> {
          ...
          Stack(
            children: [
              Container(
                  ...
                  child: Text(ret.formattedAddress != null ? ret.formattedAddress! : "", style: theme.text14bold,)
              ),
              Positioned.fill(
                child: Material(
                      ...
                      child: Container(
                          ...
                          Text(
                            (_currentPos != null ) ? "$_textAddress \n ${_currentPos!.latitude.toStringAsFixed(6)}, "
                                "${_currentPos!.longitude.toStringAsFixed(6)}" : "${strings.get(181)}",
  ...
  _addMarker(double lat, double lng){
    MarkerId _lastMarkerId = MarkerId("addr$lat");
    final marker = Marker(
  ...
  _selectPos(LatLng pos){
    markers.clear();
    _currentPos = pos;
    MarkerId _lastMarkerId = MarkerId("addr${pos.latitude}");
    final marker = Marker(
        markerId: _lastMarkerId,
