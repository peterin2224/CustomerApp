...
import 'package:fooddelivery/model/server/cancel.dart';
...
class OrderDetailsScreen extends StatefulWidget {
  ...
  OrderDetailsScreen({Key? key, required this.onBack}) : super(key: key);
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  _arrived(){
    _waits(true);
    curbSidePickupArrived(idOrder, account.token, _arrivedSuccess, _error);
  }

  _cancel(){
    _waits(true);
    cancelOrder(idOrder, account.token, _cancelSuccess, _error);
  }

  _openCancel() {
    _dialogBody = Column(
      children: [
        Text(strings.get(320), style: theme.text14,), /// "Are you sure?",
        SizedBox(height: 40,),
        Row(children: [
          Expanded(child: IButton3(
              color: Colors.red,
              text: strings.get(321),              /// "Cancel order",
  ...
  _body(){
    ...
    for (var item in ordersData){
      if (item.orderid == idOrder) {
        ...
        if (_status != 5 && _status != 6 && item.arrived != "true")
          list.add(Container(
              width: windowWidth/2,
              child: IButton3(
                  color: theme.colorPrimary,
