...
class Response {
  ...
  Response({required this.error, required this.favorites, required this.food, required this.currency});
  factory Response.fromJson(Map<String, dynamic> json){
    List<FavoritesData> m = [];
    if (json['favorites'] != null) {
      var items = json['favorites'];
      var t = items.map((f)=> FavoritesData.fromJson(f)).toList();
      m = t.cast<FavoritesData>().toList();
    }
    List<DishesData> _food = [];
    if (json['food'] != null) {
      var items = json['food'];
...
class FavoritesData {
  String food;
  FavoritesData({required this.food});
